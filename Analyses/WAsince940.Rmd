---
title: "Homicides by Police in WA since Initiative 940"
...
all.cases <- dim(finalmerge_940)[1]
...
last.url <- homicides$url_click[last.case]
  if(is.na(last.url) & grepl("Spencer", last.name)) {
  last.url <- '<a href="https://amp.thenewstribune.com/news/local/crime/article254318018.html">https://amp.thenewstribune.com/news/local/crime/article254318018.html</a>'}
...
## we use "mo" for numeric month and "month" for month abbreviation

curr_mo <- lubridate::month(Sys.Date())
curr_yr <- lubridate::year(Sys.Date())

start_mo <- 12  # just December 2018
start_yr <- 2018
...
## set up first year (2018, only Dec)
...
## tack on last months/year for current year
...
After winning over 60% of the statewide vote, Initiative 940 was passed into WA state law on December 6, 2018.  This report tracks the number of persons killed by police in WA state since that date.


## MOST RECENT DATA UPDATE (`r last.data.update`):


* __Total homicides by police since I-940 was enacted into law: 
`r last.case`__

* __Last reported case: 
`r last.name`, age `r last.age`, on `r last.date` by `r last.agency`__

    `r ifelse(last.name == "(Name not released)", "This", last.name)` is the `r paste0(tot.this.yr, num.suffix)` person killed by police in `r as.numeric(names(tot.by.yr[length(tot.by.yr)]))`.  The cause of death is reported as `r last.cod`.  `r last.url`
...
This report is restricted to the cases that can be classified as homicides by police, it excludes cases identified in the Fatal Encounters dataset as suicides, and a handful of other cases (see the [last section](#dataset-comparison) for details).

___

## What is a homicide?

The deadly force incidents in this report are homicides.  A homicide is  defined simply as the killing of one person by another.  In the context of this report it refers to any encounter with law enforcement officers that results in a fatality.  Homicides normally result in a criminal investigation or inquest, but the word does not imply a crime has been committed.
...
homicides %>%
  ...
  labs(title = "Fatalities by Race",
       caption = "WA State since 940; y-axis=pct, bar label=count") +
         xlab("Reported Race") +
...
# Trend over time {.tabset}

For all of these plots, we exclude the December 2018 data.  Each plot is intended to allow monthly or annual comparisons.  But December 2018 is not comparable, because it includes cases for only the part of the month after December 6th, when the law passed, and also doesn't represent a full annual total.

## By month

This plot shows the number of people killed by police each month since the passage of I-940. 
...
p <- ggplot(dat, aes(x = index.date, 
  ...
  scale_x_continuous(breaks = dat$index.date[seq(1, nrow(dat), 3)], 
                     label = dat$mo.yr[seq(1, nrow(dat), 3)])  +
...
This plot highlights key dates for subsequent legislation and legal actions. 

* The $\color{blue}{\textsf{blue lines}}$ are legislative/regulatory actions  (940 WACs, first bill passes in 2021, first & last bills signed by Gov 2021)
...
* This is an interactive plot:  You can hover over points to get the  number of cases for that month and year, and hovering at the top or bottom of the vertical lines will identify the action.
...
p <- ggplot(dat, aes(x = index.date, 
  ...
  geom_vline(aes(xintercept = as.numeric(lubridate::ymd("2019-01-01")),
                 text = ""),
             col="grey", alpha=.8) +
...
this.yr.col <- c("2019-2020"="lightsteelblue", "2021"="black")
this.yr.alpha <- c("2019-2020"= 0.5, "2021"= 1)
...
df <- homicides %>%
  ...
  mutate(count = tidyr::replace_na(count, 0),
         ...
         this.yr = factor(ifelse(year==curr_yr, "2021", "2019-2020"),
...
p <- ggplot(data = df,
  ...
  labs(title = "WA since I-940:  Monthly totals with moving average",
...
The total number of cases in the Fatal Encounters dataset for WA State since I-940 took effect, is
`r dim(finalmerge_940)[1]`.  These are classified as follows:
...
tab <- finalmerge_940 %>%
  ...
  summarize(Number = n(),
            Percent = round(100*Number/nrow(finalmerge_940), 1)
  ) %>%
  arrange(desc(Number)) %>%
...
The Fatal Encounters data includes some cases described as "suicides" that occur during an encounter with police.  **We have excluded these from the analyses in this report.**  However, we acknowledge that these cases warrant further assessment.  The classification as "suicide" is not definitive, it is typically based on the narrative provided by law enforcement.  And even if these are suicides, it is possible that the outcome might have been different if the case had been handled differently, for example, by calling in a crisis intervention team.

The excluded case counts and fraction of total cases by year is shown in the table below.  Note that 2018 includes only one partial month -- after Dec 6 when the I-940 law was passed.
...
tab <- finalmerge_940 %>%
  ...
  bind_rows(data.frame(year ="Total", 
                       Number = sum(.$Number), 
                       Percent = round(100*sum(.$Number)/nrow(finalmerge_940), 1)))
