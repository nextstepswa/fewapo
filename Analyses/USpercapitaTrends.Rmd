...
homicides <- fe_clean %>%
  filter(year < last_complete_yr) %>% # restrict to last complete yr 
  filter(cod != "suicide") %>%
...
Since we are working with annual summary metrics, our last full year of data from the Fatal Encounters Project is `r last_complete_yr`.  For the census population data, we extrapolate the growth rate from 2018-2019 (by age, race, sex and state) to estimate the 
`r last_complete_yr` population.
...
### State age distributions in `r last_complete_yr` {.tabset .tabset-pills}
...
```{r}
p<- ggplot(data = pop.age.dist %>% filter(year==last_complete_yr),
...
It's worth getting a sense of the difference in population size across states.  We use `r last_complete_yr` for comparison below.

```{r statepopsize, message=F, warning=F}
p <- ggplot(data = popdata$pop.st.yr %>% filter(year==last_complete_yr),
  ...
  labs(title = paste("State populations ", last_complete_yr),
       x="Population size",
       y="State")
...
change <- df.all %>%
  filter(year< 2006 | year>2014 & year !=last_complete_yr) %>%
...
We calculate an average over the first 5 years (start, 2000-2005), and the last 5 years (end, 2015-`r last_complete_yr`) to reduce the year-to-year variability and get a more stable estimate of the changes in risk over the two decades.
...
p <- ggplot(df.st,
  ...
  labs(title="Risk of being killed by police by state: 2000-5 vs 2015-20",
       ...
       y = paste0("persons killed per M residents 2015-", last_complete_yr, "average"),
...
p <- ggplot(df.all, aes(x=year, y=fe.rate, group=st, 
  ...
  scale_color_brewer(palette="Spectral", direction= -1) +
  labs(title=paste0("Trends in the risk of being killed by police by state: 2000-",
                    last_complete_yr),
...
p <- ggplot(df.st,
  ...
  labs(title=paste0("Relative changes in population and fatalities: 2000-",
                   last_complete_yr),
...
This shows the average rate for the 2015-`r last_complete_yr` period for each state.
...
p <- ggplot(data = df.st,
  ...
  scale_color_manual(values = c("lightsteelblue3", "gold")) +
  theme(axis.text.y=element_text(size=rel(.5)), legend.position = "none") +
  labs(title = paste0("State rankings:  2015-", last_complete_yr, "average"),
       x="Number of persons killed per million residents",
       y="State")
...
p <- ggplot(data = df.st,
  ...
  scale_color_manual(values = c("lightsteelblue3", "gold")) +
  theme(axis.text.y=element_text(size=rel(.5)), legend.position = "none") +
  labs(title = paste0("State rankings:  Change in risk 2000-", last_complete_yr),
