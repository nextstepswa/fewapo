---
title: "State trends in the risk of being killed by police 2000-2020"
...
load(file = here("data-outputs", "StatePop2000-2021.rda"))

curr_yr <- lubridate::year(max(fe_clean$date)) #most recent data from FE

homicides <- fe_clean %>%
  filter(year < curr_yr) %>% # restrict to last complete yr 
  filter(cod != "suicide")
...
Since we are working with annual summary metrics, our last full year of data from the Fatal Encounters Project is 2020.  For the census population data, we extrapolate the growth rate from 2018-2019 (by age, race, sex and state) to estimate the 2020 population.
...
ggplot() +
  ...
  labs(title="Age distribution of people killed by police: 2000-2020",
       fill = "Persons killed",
       color = "US Population") +
...
### State age distributions in 2020 {.tabset .tabset-pills}
...
```{r}
p<- ggplot(data = pop.age.dist %>% filter(year==2020),
...
### State trends in age composition 2000-2020 {.tabset .tabset-pills}
...
It's worth getting a sense of the difference in population size across states.  We use 2020 for comparison below.

```{r statepopsize, message=F, warning=F}
p <- ggplot(data = popdata$pop.st.yr %>% filter(year==2020),
  ...
  labs(title = "State populations 2020",
       x="Population size",
       y="State")
...
We calculate an average over the first 5 years (start, 2000-2005), and the last 5 years (end, 2015-2020) to reduce the year-to-year variability and get a more stable estimate of the changes in risk over the two decades.
...
p <- ggplot(df.st,
  ...
  labs(title="Risk of being killed by police by state: 2000-5 vs 2015-20",
       ...
       y = "persons killed per M residents 2015-2020 average",
...
p <- ggplot(df.all, aes(x=year, y=fe.rate, group=st, 
  ...
  scale_color_brewer(palette="Spectral", direction= -1) +
  labs(title="Trends in the risk of being killed by police by state: 2000-2020",
...
p <- ggplot(df.st,
  ...
  labs(title="Relative changes in population and fatalities: 2000-2020",
...
This shows the average rate for the 2015-2020 period for each state.
...
p <- ggplot(data = df.st,
  ...
  scale_color_manual(values = c("lightsteelblue3", "gold")) +
  theme(axis.text.y=element_text(size=rel(.5)), legend.position = "none") +
  labs(title = "State rankings:  2015-2020 average",
       x="Number of persons killed per million residents",
       y="State")
...
p <- ggplot(data = df.st,
  ...
  scale_color_manual(values = c("lightsteelblue3", "gold")) +
  theme(axis.text.y=element_text(size=rel(.5)), legend.position = "none") +
  labs(title = "State rankings:  Change in risk 2000-2020",
...
```{r map2020, message=F, warning=F, fig.height=4}
