...
Here we report the annual totals up through the last complete year --  `r last_complete_yr` -- with data from the Washington Post data only, since the Fatal Encounters project stopped updating at the end of 2021.  Recall that Washington Post data only include persons shot and killed by police, while Fatal Encounters included all persons who are killed by police (including deaths due to asphyxiation, tasers, vehicle chases, etc).
...
pop2020 <- read_csv("https://www2.census.gov/programs-surveys/popest/datasets/2010-2020/state/totals/nst-est2020-alldata.csv") %>%
  filter(SUMLEV != "020") %>%
  select(NAME, contains("POPESTIMATE"))
  
pop2023 <- read_csv("https://www2.census.gov/programs-surveys/popest/datasets/2020-2023/state/totals/NST-EST2023-ALLDATA.csv") %>%
  filter(SUMLEV %in% c("010", "040")) %>%
  select(NAME, contains("POPESTIMATE"))

pop <- left_join(pop2020, pop2023, by = "NAME") %>%
  rowwise() %>%
  mutate(POPESTIMATE2020 = mean(c_across(contains("2020")))) %>%
  select(NAME, POPESTIMATE2015:POPESTIMATE2019, POPESTIMATE2020, POPESTIMATE2021:POPESTIMATE2023) %>%
...
Here we report the WaPo data by state for the most recent complete year: `r last_complete_yr`.


### Number shot

* States are ranked by the the number of persons shot in `r last_complete_yr`.
...
p <- ggplot(data = states.all %>% filter(!grepl("United|Other", state)),
            aes(x=shot2023, y=reorder(state, shot2023), 
                fill = shot2023,
  ...
  scale_fill_gradient2(
    ...
    mid = "white", 
    high = "red", 
    midpoint = median(states.all$shot2023)) +
  #scale_x_continuous(labels = scales::percent) +
  
  theme(axis.text.y=element_text(size=rel(.7)), legend.position = "none") +
  labs(title = paste("State ranking: Number of persons shot by police"),
       x="Number shot",
       y="State")
...
* States are ranked by the population adjusted rate of persons shot in `r last_complete_yr`:  the rate per million state residents.
...
p <- ggplot(data = states.all %>% filter(!grepl("United|Other", state)),
            aes(x=rateshot2023, y=reorder(state, rateshot2023), 
                fill = rateshot2023,
                color = factor(ifelse(state=="Washington", 1, 0)),
                text=paste(state, 
                           "total:", round(rateshot2023, 1)))
            ) +
  ...
  scale_fill_gradient2(
    ...
    mid = "white", 
    high = "red", 
    midpoint = median(states.all$rateshot2023)) +
  #scale_x_continuous(labels = scales::percent) +
  
  theme(axis.text.y=element_text(size=rel(.7)), legend.position = "none") +
  labs(title = paste("State ranking: Population adjusted rate of persons shot by police"),
...
* States are ranked by the percent change in the number of persons shot from `r last_complete_yr-1` to `r last_complete_yr`.
...
df <- states.all %>% 
  ...
  select(state, shot2022:shot2023) %>%
  mutate(pct.chg = shot2023/shot2022 -1)
...
p <- ggplot(data = df,
            aes(x=pct.chg, y=reorder(state, pct.chg), 
                ...
                color = factor(ifelse(state=="Washington", 1, 0)),
                text=paste0(state, 
                           paste("<br>", "number shot in ", last_complete_yr, ": ", shot2023),
  ...
  theme(axis.text.y=element_text(size=rel(.7)), legend.position = "none") +
  labs(title = paste("State ranking: Percent change in number of persons shot by police from previous year"),
       x="Percent change",
       y="State")
