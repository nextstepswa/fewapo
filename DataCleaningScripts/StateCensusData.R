...
# 2010-19
# Single Yr Age, Sex, Race, and Hispanic Origin - 5 race groups for States only
# (5 race alone or in combination groups) (SC-EST2019-ALLDATA5)
...
## For 2010s we will construct a proto object first to allow for an age
## distribution table

p2010a <- pop.st.ars_10_19 %>%
  filter(REGION !=0 & DIVISION !=0 & STATE !=0 & 
           SEX != 0 & ORIGIN != 0 & RACE !=0) %>% # remove totals
...
## Here we also project to 2021, assuming growth rate from 2018-19
p2010 <- p2010a %>%
  group_by(STATE, NAME, REGION, DIVISION, SEX, ORIGIN, RACE, age.grp) %>%
  summarize(across(starts_with("POP"), ~sum(.x, na.rm = TRUE))) %>%
  mutate(growth_rate = ifelse(POPESTIMATE2018==0, 0, 
                              as.numeric(POPESTIMATE2019)/as.numeric(POPESTIMATE2018)),
         POPESTIMATE2020 = POPESTIMATE2019 * growth_rate,
         POPESTIMATE2021 = POPESTIMATE2020 * growth_rate)

#-------------------------------------------------------------------------------------
  
# Overall US age distribution in the 2010s ----
pop.agedist.2010s <- p2010a %>%
  group_by(AGE) %>%
  summarize(across(starts_with("POP"), ~sum(.x, na.rm = TRUE))) %>%
  rowwise() %>%
...
popdata <- list("st.info" = st.info,
                "pop.agedist.2010s" = pop.agedist.2010s,
...
save(popdata, 
     file = here::here("data-outputs", "StatePop2000-2021.rda"))
